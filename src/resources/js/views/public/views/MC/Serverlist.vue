    ...
    <div class="container">
        <div class="columns is-multiline is-centered" v-else>
            <div class="column is-6" v-for="server in servers" :key="server.name">
                <div class="box mc-server" v-if="server.online">
                    <p class="title server-name is-5 mc-text">{{ server.name }}<span class="ping">{{ server.status.players.online }}/{{ server.status.players.max }}</span></p>
                    <span class="subtitle is-5 mc-text" v-html="server.status.description.text">
                    </span>
                </div>
                <div class="box mc-server" v-else>
